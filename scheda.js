(function () {
  ...
  window.IanneceAPI.fetchPerson(id).then(({ person }) => {
    ...
    content.innerHTML = `<div class="p-4"><section class="bg-[#4a5d4e] rounded-3xl p-8 text-center text-white relative overflow-hidden${person.foto ? '' : ' min-h-[21.75rem]'}"><h2 class="text-3xl font-bold mb-1">${esc(name)}</h2>${available(person.annoNascita) ? `<p class="text-white/80 text-sm mb-2">${esc(person.annoNascita)}</p>` : ''}${person.rami?.length ? `<p class="text-[#e7e0cf] text-xs font-bold uppercase mb-8">${esc(person.rami.map(branch => branch.nome).join(', '))}</p>` : '<div class="mb-8"></div>'}${person.foto ? `<button id="photo-button" class="w-48 h-48 mx-auto rounded-2xl border-4 border-white/30 bg-white/10 overflow-hidden flex items-center justify-center shadow-2xl"><img src="${esc(person.foto)}" class="w-full h-full object-cover" alt="Ritratto di ${esc(name)}"></button>` : ''}</section></div><div class="px-4 mb-6"><section class="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm"><h3 class="text-xl font-bold mb-3">Notizie</h3><p class="text-gray-600 text-sm whitespace-pre-line">${show(person.note)}</p></section></div><div class="px-4 mb-6"><section class="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm"><h3 class="text-xl font-bold mb-4">Dati personali</h3><dl class="space-y-4">${personalDetails}</dl></section></div>${familySections ? `<div class="px-4 mb-6"><h3 class="text-xl font-bold mb-4">Legami familiari</h3><div class="space-y-5">${familySections}</div></div>` : ''}${person.matrimoni?.length ? `<div class="px-4 mb-6"><h3 class="text-xl font-bold mb-4">Matrimoni</h3><div class="space-y-3">${person.matrimoni.map(marriage => `<section class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden"><div class="p-5"><h4 class="font-bold">${show(marriage.titolo)}</h4><p class="text-xs text-gray-500 mt-2">${show([formatDate(marriage.data) || marriage.anno, marriage.luogo].filter(available).join(' · '))}</p>${marriage.note ? `<p class="text-sm text-gray-600 mt-2">${esc(marriage.note)}</p>` : ''}</div>${marriage.coniugi?.length ? `<div class="border-t border-gray-50 divide-y divide-gray-50">${marriage.coniugi.map(spouse => relation(spouse, 'Coniuge')).join('')}</div>` : ''}</section>`).join('')}</div></div>` : ''}${person.rami?.length ? `<div class="px-4 mb-6"><a href="rami.html" class="w-full bg-[#f0f3ff] text-[#4a5d4e] py-4 rounded-2xl font-bold flex justify-center gap-2"><span class="material-symbols-outlined">family_history</span>Vedi ramo familiare</a></div>` : ''}<div class="px-4 mb-8"><a href="albero.html?id=${encodeURIComponent(person.id)}" class="w-full bg-[#f0f3ff] text-[#4a5d4e] py-4 rounded-2xl font-bold flex justify-center gap-2"><span class="material-symbols-outlined">account_tree</span>Vedi nell’albero</a></div>`;
