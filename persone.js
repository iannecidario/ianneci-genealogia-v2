(function () {
  ...
  function visible() {
    ...
    const matches = people.filter(person => !branch || person.ramoIds.includes(branch));
    return query ? window.IannecePersonSearch.filterAndSort(matches, query) : matches;
  }
  ...
  function render() {
    ...
    const groups = search.value.trim() ? new Map([['Risultati', items]]) : new Map();
    if (!search.value.trim()) for (const person of items) {
